...
给定n个作业的集合{J1, J2, ..., Jn}。每个作业必须先由机器1处理, 然后再由机器2处理。作业Ji在机器j上的处理时间为$t_{ji}$。对于一个确定的作业调度, 设$F_{ji}$是作业i在机器j上完成处理的时间。所有作业在机器2上完成处理的时间和称为该作业调度的完成时间。设计一个算法, 对给定的n个作业的集合, 求出完成时间最短的作业调度。
